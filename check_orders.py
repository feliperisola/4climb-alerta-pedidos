...
def save_state(state, sha=None):
    ...
    payload = {
        'message': f'chore: atualiza estado do alerta [{datetime.now().strftime("%Y-%m-%d %H:%M")}]',
        'content': content,
    }
    if sha:
        payload['sha'] = sha
...
def send_alert_email(count):
    ...
    text = f"""Alerta de pedidos — 4climb

Existem {count} pedidos em aberto no Tiny.
Verificado em: {now}
...
<h2 style="color:#d32f2f">⚠️ Alerta de Pedidos — 4climb</h2>
<p>Existem <strong style="font-size:1.4em;color:#d32f2f">{count}</strong> pedidos em aberto no Tiny.</p>
...
def main():
    print(f"Verificando pedidos em aberto... [{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}]")

    count = get_open_orders_count()
    print(f"Pedidos em aberto: {count} (limite: {THRESHOLD})")
